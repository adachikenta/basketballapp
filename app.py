...
class PlayDiagram(db.Model):
    ...
    user = db.relationship(
        "User",
        backref=db.backref("play_diagrams", lazy="dynamic")
    )
...
def play_list():
    """籠球図一覧の表示"""
    plays = PlayDiagram.query.filter_by(
        user_id=current_user.id
    ).order_by(
        PlayDiagram.updated_at.desc()
    ).all()
...
def play_new():
    """新しい籠球図を作成"""
    return render_template("play_diagram.html")
...
@app.route("/api/play/save", methods=["POST"])
@login_required
def save_play():
    """籠球図の保存"""
    try:
        data = request.json
        play_name = data.get("name")
        play_description = data.get("description", "")
        diagram_data = data.get("diagram_data")
        if not play_name:
            return jsonify(
                {"status": "error", "message": "プレー名は必須です"}
            ), 400
        if not diagram_data:
            return jsonify(
                {"status": "error", "message": "図データが必要です"}
            ), 400
        # 新規作成
        play = PlayDiagram(
            user_id=current_user.id,
...
@app.route("/api/play/<play_id>")
@login_required
def get_play(play_id):
    """保存された籠球図を取得"""
    play = PlayDiagram.query.filter_by(
        id=play_id,
        user_id=current_user.id
    ).first()
    if not play:
        return jsonify(
            {"status": "error", "message": "籠球図が見つかりません"}
        ), 404
    return jsonify(
        {
            "id": play.id,
...
@app.route("/api/play/delete/<play_id>", methods=["DELETE"])
@login_required
